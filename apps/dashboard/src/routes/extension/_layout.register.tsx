...
import { extensionAuthRedirectBaseURL } from '../../config/api'
...
export const Route = createFileRoute('/extension/_layout/register')({
...
function RegisterPage() {
  ...
  const handleRegister: SubmitHandler<IRegisterSchema> = async ({
    email,
    password,
  }) => {
    try {
      ...
      toast.success(
        'You have been registered, this tab will close automatically...',
      )
    } catch (error) {
  ...
  const handleRegisterWithGoogle = async () => {
    try {
      await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: extensionAuthRedirectBaseURL,
        },
      })
    } catch (error) {
      toast.error(unwrapError(error, ErrorMessages.GenericError))
  ...
  return (
    <div className="min-h-screen w-full flex flex-col items-center justify-center p-10 bg-background-light overscroll-none">
      <div className="w-full max-w-md flex flex-col items-center bg-card-background px-10 py-10 rounded-2xl shadow-sm">
        ...
        <p className="text-sm text-center mt-1 text-text-muted">
        ...
        >
          ...
          <p className="mt-3 text-xs font-medium text-accent-dark">
            By signing up, I agree to the Terms of Service, acknowledge
            Screech's Privacy Policy.
          </p>
        </form>
