...
import { assert } from '@rekorder.io/utils';
...
export class BaseRecorder<T extends Muxer> {
  ...
  private scaleFitContent(videoWidth: number, videoHeight: number, containerWidth: number, containerHeight: number) {
    ...
    this.audioWritableStream = new WritableStream<AudioData>(
      {
        write: async (data) => {
          ...
          if (this.recording) {
            assert(this.audioEncoder, 'Audio encoder is not initialized');
  ...
  protected handleCaptureCanvasFrame() {
    ...
    assert(this.videoEncoder, 'Video encoder is not initialized');
  ...
  async handleSetupCanvasWriter() {
    assert(this.videoReadableStream, 'Video readable stream is not initialized');
    ...
    this.videoWritableStream = new WritableStream<VideoFrame>(
      {
        write: async (data) => {
          const frame = this.clone ? data.clone() : data;
          const bounds = this.scaleFitContent(frame.codedWidth, frame.codedHeight, this.canvas.width, this.canvas.height);
          this.context.drawImage(frame, bounds.x, bounds.y, bounds.width, bounds.height);
          frame.close();
        },
        abort: (reason) => {
  ...
  async handleSaveStream() {
    ...
    if (this.audioEncoder) {
      try {
        ...
        console.warn('Failed to flush audio encoder:', error);
      }
    }
    ...
    assert(this.muxer, 'Muxer is not initialized');
    this.muxer.finalize();
