import type { RuntimeMessage } from '@rekorder.io/types';
...
class MP4Worker {
  ...
  async handleSeek(type: 'frame' | 'time', value: number) {
    ...
    if (!chunk) {
      import('./constants/events').then(({ VideoPlayerEvents }) => self.postMessage({ type: VideoPlayerEvents.SeekVideoError, payload: { error: 'No chunk found' } }));
      return;
    }
    ...
    if (chunk.type === 'key') {
      ...
      if (!result.chunk) {
        import('./constants/events').then(({ VideoPlayerEvents }) => self.postMessage({ type: VideoPlayerEvents.SeekVideoError, payload: { error: 'No key frame found' } }));
        return;
      }
  ...
  async handlePaintNextFrame() {
    if (this.frameIndex >= this.videoMetadata.frames) {
      import('./constants/events').then(({ VideoPlayerEvents }) => self.postMessage({ type: VideoPlayerEvents.NextFrameError, payload: { error: 'End of frames reached' } }));
      return;
    }
    ...
    if (chunk) {
      while (true) {
        if (retries >= maxRetries) {
          import('./constants/events').then(({ VideoPlayerEvents }) => self.postMessage({ type: VideoPlayerEvents.NextFrameError, payload: { error: 'Timed out while decoding frame' } }));
          break;
        }
